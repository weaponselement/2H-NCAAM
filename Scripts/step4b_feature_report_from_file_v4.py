...
def score_snapshot(play):
    return safe_int(play.get("homeScore")), safe_int(play.get("visitorScore"))
...
def compute_live_game_state(plays, home_tid, away_tid):
    ...
    for idx, p in enumerate(plays):
        ...
        home_score, away_score = score_snapshot(p)
        ...
        if pts and tid:
            ...
            if last_turnover_live and last_turnover_team and last_turnover_team != tid:
                teams[tid]["live_ball_points_allowed"] += 0
        ...
        if home_score is not None:
            home_prev = home_score
        if away_score is not None:
            away_prev = away_score
    ...
    if home_stats and away_stats:
        ...
        output["game"]["pace_profile"] = (
            "run_and_gun" if combined_poss >= 36 else "moderate" if combined_poss >= 32 else "grinder"
        )
...
def baseline_aggregate(data_root: str, team_seo: str, game_ids):
    ...
    if [v for v in dep_vals if v is not None]:
        agg["top2_share"] = {
            "mean": round(mean([v for v in dep_vals if v is not None]), 3),
            "min": min([v for v in dep_vals if v is not None]),
            "max": max([v for v in dep_vals if v is not None]),
        }
    return per_game, agg
...
def synthesize_game(game_id: str, home_seo: str, away_seo: str, home_ht: int, away_ht: int, home_live: dict, away_live: dict, home_base: dict, away_base: dict, game_meta: dict):
    ...
    home_orb_edge = home_live.get("ORB", 0) - away_live.get("DRB", 0)
    away_orb_edge = away_live.get("ORB", 0) - home_live.get("DRB", 0)
    ...
    if home_live.get("second_chance_points", 0) > away_live.get("second_chance_points", 0):
        structural_home += 0.75
        reasons.append(f"{home_seo} created more second-chance scoring")
    elif away_live.get("second_chance_points", 0) > home_live.get("second_chance_points", 0):
    ...
    final_total_mid = (home_ht or 0) + (away_ht or 0) + expected_2h
    total_range = f"{final_total_mid - 6}-{final_total_mid + 6}"

    biggest_uncertainty = "Whether 1H scoring came from repeatable creation or from fragile shot variance."
    if home_dep and home_dep >= 0.68:
    ...
    elif away_dep and away_dep >= 0.68:
    ...
    if abs(home_to - away_to) >= 4:
        what_flips = "If the team currently losing the turnover battle cleans that up, the game state can flip quickly."
    ...
    return {
        ...
        "second_half_points_projection": {
            "mid": expected_2h,
            "range": f"{expected_2h - 5}-{expected_2h + 5}",
            "variance": variance,
        },
        "full_game_total_projection": {
            "mid": final_total_mid,
            "range": total_range,
            "lean": "OVER" if expected_2h >= 70 else "UNDER" if expected_2h <= 64 else "NO STRONG LEAN",
        },
...
def main():
    ...
    selected_path = args.selected_games or os.path.join(data_root, "processed", "selected_games", f"selected_games_{run_date}.json")
    ...
    if not sel:
        print(f"WARNING: GameID {args.game_id} not found in {selected_path}")
    print("Proceeding without selected-games metadata...")
    ...
    out_path = os.path.join(out_dir, f"feature_report_v4_{args.game_id}_{ts}.json")
