...
# Inputs (these already exist from your earlier steps)
...
def extract_fouler_name_from_text(desc: str):
    """
    Robust foul-name extractor that avoids fragile character classes.
    Examples seen:
      "Personal Foul on Michigan's Nimari Burnett (Cade Tyson draws the foul)"
      "Offensive foul by Michigan's Yaxel Lendeborg"
      "William Kyle III shooting foul (Isaiah Evans draws the foul)"
    Strategy:
      Capture whatever appears after "<on|by> <Team>'s " up to "(" or end.
    """
    if not desc:
        return ""

    # Try "on TEAM's NAME" or "by TEAM's NAME"
    ...
    # Try "NAME shooting foul" pattern (no TEAM's)
    m = re.search(r"^(.+?)\s+(?:shooting|personal|offensive)\s+foul", desc, flags=re.I)
    if m:
        name = m.group(1).strip()
        name = re.sub(r"\s+", " ", name)
        return name
...
def compute_features_for_team(plays, team_id):
    ...
    for p in plays:
        ...
        if cat.startswith("foul_"):
            ...
            # Prefer firstName/lastName if present; else parse safely from text
            name = normalize_name(p.get("firstName"), p.get("lastName"))
            if not name:
                name = extract_fouler_name_from_text(desc)

            if name:
                f["player_fouls"][name] = f["player_fouls"].get(name, 0) + 1

        # points + player points (scorer usually provided in firstName/lastName)
...
def extract_first_half_plays_from_baseline_payload(payload):
    """
    Confirmed baseline schema: periods[].playbyplayStats with periodNumber==1 for first half.
    """
...
def team_id_from_baseline_payload(payload, team_seo):
    """
    Map a team seo (e.g., 'michigan') to numeric teamId from baseline payload teams[].seoname.
    """
...
def main():
    import sys
    if len(sys.argv) < 2:
        print("Usage: python step4b_feature_report_from_file_v3.py <gameID>")
        print("Example: python step4b_feature_report_from_file_v3.py 6503505")
        return
    ...
    # Baseline + selected games to determine home/away seos
